...
const DailyTasks = () => {
  ...
  return (
    <div className="max-w-2xl mx-auto my-8 bg-slate-800 rounded-lg p-6">
      ...
      <ul className="list-none p-4 space-y-2 bg-slate-700 rounded-lg">
        {tasks.map(task => (
          // List item with a darker hover effect
          <li 
            key={task.id} 
            className="flex items-center p-3 rounded-md transition-colors duration-200 hover:bg-slate-600"
          >
            <input
              type="checkbox"
              checked={task.completed}
              onChange={() => handleToggleTask(task.id)}
              className="h-5 w-5 mr-4 rounded border-gray-400 bg-slate-700 text-indigo-500 focus:ring-indigo-500 cursor-pointer"
            />
            {/* Conditional styling for the task text with lighter colors */}
            <span className={`text-base flex-grow ${task.completed ? 'line-through text-gray-400' : 'text-gray-200'}`}>
              {task.text}
            </span>
          </li>
        ))}
